...
# Path of results
path_results = '/Users/binfang/Documents/SMAP_Project/results/results_200205'
# Path of preview
...
hdf_file = path_model + '/ds_model_coef_nofill.hdf5'
...
for iyr in year_plt:  # range(yearname):
    for imo in range(len(month_plt)):  # range(len(monthname)):
        hdf_file_smap_9km = path_model + '/smap_sm_9km_' + str(iyr) + monthname[month_plt[imo]-1] + '.hdf5'
...
fig = plt.figure(figsize=(13, 8), facecolor='w', edgecolor='k')
for ipt in range(len(monthname)//3):
    ax = fig.add_subplot(2, 2, ipt+1, projection=ccrs.PlateCarree(), extent=[-180, 180, -70, 90])
    ax.add_feature(shape_world, linewidth=0.5)
    ...
    ax.set_title(title_content[ipt], pad=20, fontsize=17, weight='bold')
...
for iyr in year_plt:  # range(yearname):
    for imo in range(len(month_plt)):  # range(len(monthname)):
        hdf_file_smap_9km = path_smap + '/smap_sm_9km_' + str(iyr) + monthname[month_plt[imo]-1] + '.hdf5'
        f_read_smap_9km = h5py.File(hdf_file_smap_9km, "r")
        varname_list_smap_9km = list(f_read_smap_9km.keys())
...
# 3.3 Lower Mekong RB
path_shp_lm = path_gis_data + '/lmrb_boundary'
os.chdir(path_shp_lm)
shp_lm_file = "mrc_SDE_WATER_b_catlmb50_wgs.shp"
shp_lm_ds = ogr.GetDriverByName("ESRI Shapefile").Open(shp_lm_file, 0)
shp_lm_extent = list(shp_lm_ds.GetLayer().GetExtent())

#Subset the region of LMRB (9 km)
[lat_9km_lm, row_lm_9km_ind, lon_9km_lm, col_lm_9km_ind] = \
    coordtable_subset(lat_world_ease_9km, lon_world_ease_9km, shp_lm_extent[3], shp_lm_extent[2], shp_lm_extent[1], shp_lm_extent[0])

# Load and subset SMAP 9 km SM of LM RB
year_plt = [2019]
month_plt = [1]
days_begin = 1
days_end = 24
...
for iyr in year_plt:  # range(yearname):
    for imo in range(len(month_plt)):  # range(len(monthname)):
        ...
        smap_9km_load = np.empty((len(row_lm_9km_ind), len(col_lm_9km_ind), days_n*2))
        smap_9km_load[:] = np.nan
        for idt in range(days_begin-1, days_end):
            smap_9km_load[:, :, 2*idt+0] = f_read_smap_9km[varname_list_smap_9km[0]][row_lm_9km_ind[0]:row_lm_9km_ind[-1]+1,
                col_lm_9km_ind[0]:col_lm_9km_ind[-1]+1, idt] # AM
            smap_9km_load[:, :, 2*idt+1] = f_read_smap_9km[varname_list_smap_9km[1]][row_lm_9km_ind[0]:row_lm_9km_ind[-1]+1,
                col_lm_9km_ind[0]:col_lm_9km_ind[-1]+1, idt] # PM
        f_read_smap_9km.close()
...
#Load and subset the region of GB RB (SMAP 1 km)
[lat_1km_lm, row_lm_1km_ind, lon_1km_lm, col_lm_1km_ind] = \
    coordtable_subset(lat_world_ease_1km, lon_world_ease_1km, shp_lm_extent[3], shp_lm_extent[2], shp_lm_extent[1], shp_lm_extent[0])

smap_1km_load = np.empty((len(row_lm_1km_ind), len(col_lm_1km_ind), days_n*2))
smap_1km_load[:] = np.nan
...
for iyr in year_plt:  # range(yearname):
    for imo in range(len(month_plt)):  # range(len(monthname)):
        for idt in range(days_n): # 16 days
            ...
            tif_file_smap_1km = path_smap_sm_ds + '/' + str(iyr) + '/smap_sm_1km_ds_' + str(iyr) + str_doy.zfill(3) + '.tif'
            src_tf = gdal.Open(tif_file_smap_1km)
            src_tf_arr = src_tf.ReadAsArray()[:, row_lm_1km_ind[0]:row_lm_1km_ind[-1]+1,
                         col_lm_1km_ind[0]:col_lm_1km_ind[-1]+1]
            src_tf_arr = np.transpose(src_tf_arr, (1, 2, 0))
            smap_1km_load[:, :, 2*idt+0] = src_tf_arr[:, :, 0]
            smap_1km_load[:, :, 2*idt+1] = src_tf_arr[:, :, 1]
...
# Subplot maps
# Load in watershed shapefile boundaries
shapefile_lm = fiona.open(path_shp_lm + '/' + shp_lm_file, 'r')
crop_shape_lm = [feature["geometry"] for feature in shapefile_lm]
shp_lm_extent = list(shapefile_lm.bounds)

output_crs = 'EPSG:4326'

# Subset and reproject the SMAP SM data at watershed
# 1 km
masked_ds_lm_1km_all = []
for n in range(smap_1km_data_stack.shape[0]):
    sub_window_lm_1km = Window(col_lm_1km_ind[0], row_lm_1km_ind[0], len(col_lm_1km_ind), len(row_lm_1km_ind))
    kwargs_1km_sub = {'driver': 'GTiff', 'dtype': 'float32', 'nodata': 0.0, 'width': len(lon_world_ease_1km),
                      'height': len(lat_world_ease_1km), 'count': 1, 'crs': CRS.from_dict(init='epsg:6933'),
                      'transform': Affine(1000.89502334956, 0.0, -17367530.44516138, 0.0, -1000.89502334956, 7314540.79258289)}
    smap_sm_lm_1km_output = sub_n_reproj(smap_1km_data_stack[n, :, :], kwargs_1km_sub, sub_window_lm_1km, output_crs)

    masked_ds_lm_1km, mask_transform_ds_lm_1km = mask(dataset=smap_sm_lm_1km_output, shapes=crop_shape_lm, crop=True)
    masked_ds_lm_1km[np.where(masked_ds_lm_1km == 0)] = np.nan
    masked_ds_lm_1km = masked_ds_lm_1km.squeeze()

    masked_ds_lm_1km_all.append(masked_ds_lm_1km)

masked_ds_lm_1km_all = np.asarray(masked_ds_lm_1km_all)


# 9 km
masked_ds_lm_9km_all = []
for n in range(smap_9km_data_stack.shape[0]):
    sub_window_lm_9km = Window(col_lm_9km_ind[0], row_lm_9km_ind[0], len(col_lm_9km_ind), len(row_lm_9km_ind))
    kwargs_9km_sub = {'driver': 'GTiff', 'dtype': 'float32', 'nodata': 0.0, 'width': len(lon_world_ease_9km),
                      'height': len(lat_world_ease_9km), 'count': 1, 'crs': CRS.from_dict(init='epsg:6933'),
                      'transform': Affine(9008.05, 0.0, -17367530.44516138, 0.0, -9008.05, 7314540.79258289)}
    smap_sm_lm_9km_output = sub_n_reproj(smap_9km_data_stack[n, :, :], kwargs_9km_sub, sub_window_lm_9km, output_crs)

    masked_ds_lm_9km, mask_transform_ds_lm_9km = mask(dataset=smap_sm_lm_9km_output, shapes=crop_shape_lm, crop=True)
    masked_ds_lm_9km[np.where(masked_ds_lm_9km == 0)] = np.nan
    masked_ds_lm_9km = masked_ds_lm_9km.squeeze()

    masked_ds_lm_9km_all.append(masked_ds_lm_9km)

masked_ds_lm_9km_all = np.asarray(masked_ds_lm_9km_all)


# Make the subplot maps
title_content = ['1 km (01/01 - 01/08)', '9 km (01/01 - 01/08)', '1 km (01/09 - 01/16)', '9 km (01/09 - 01/16)',
              '1 km (01/17 - 01/24)', '9 km (01/17 - 01/24)']
feature_shp_lm = ShapelyFeature(Reader(path_shp_lm + '/' + shp_lm_file).geometries(),
                                ccrs.PlateCarree(), edgecolor='black', facecolor='none')
extent_lm = np.array(smap_sm_lm_1km_output.bounds)
extent_lm = extent_lm[[0, 2, 1, 3]]

fig = plt.figure(figsize=(8, 8), facecolor='w', edgecolor='k')
for ipt in range(3):
    # 1 km
    ax = fig.add_subplot(3, 2, ipt*2+1, projection=ccrs.PlateCarree())
    ax.add_feature(feature_shp_lm)
    ax.set_title(title_content[ipt*2], pad=20, fontsize=13, fontweight='bold')
    img = ax.imshow(masked_ds_lm_1km_all[ipt, :, :], origin='upper', vmin=0, vmax=0.5, cmap='gist_earth_r',
               extent=extent_lm)
    cbar = plt.colorbar(img, extend='both', aspect=20, pad=0.2)
    cbar.ax.tick_params(labelsize=9)
    cbar.set_label('$\mathregular{(m^3/m^3)}$', fontsize=9, x=0.95)
    gl = ax.gridlines(draw_labels=True, linestyle='--', linewidth=0.5, alpha=0.5, color='black')
    gl.xlocator = mticker.MultipleLocator(base=5)
    ...
    ax.add_feature(feature_shp_lm)
    ax.set_title(title_content[ipt*2+1], pad=20, fontsize=13, fontweight='bold')
    img = ax.imshow(masked_ds_lm_9km_all[ipt, :, :], origin='upper', vmin=0, vmax=0.5, cmap='gist_earth_r',
               extent=extent_lm)
    cbar = plt.colorbar(img, extend='both', aspect=20, pad=0.2)
    cbar.ax.tick_params(labelsize=9)
    cbar.set_label('$\mathregular{(m^3/m^3)}$', fontsize=9, x=0.95)
...
plt.subplots_adjust(left=0.1, right=0.9, bottom=0.05, top=0.92, hspace=0.35, wspace=0.25)
plt.savefig(path_results + '/sm_comp_lm_2.png')
...
#         hdf_file_smap_9km = path_model + '/smap_sm_9km_' + str(iyr) + monthname[imo-1] + '.hdf5'
#         f_read_smap_9km = h5py.File(hdf_file_smap_9km, "r")
#         varname_list_smap_9km = list(f_read_smap_9km.keys())
#
#
...
# Load in data
os.chdir(path_model)
hdf_file = path_model + '/ds_model_07.hdf5'
f_read = h5py.File(hdf_file, "r")
varname_list = list(f_read.keys())
