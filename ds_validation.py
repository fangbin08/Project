...
region_name = ['Europe', 'Australia', 'USA']
...
for ire in range(len(folder_region)): # Region folders
    ...
    for inw in range(len(folder_network)): # Network folders
        ...
        for ist in range(len(folder_site)): # Site folders
            ...
            sm_file_list = glob.glob(sm_file_path + '/*_sm_*')
        ...
        sm_mat_am = np.concatenate((np.expand_dims(np.array(stn_lat_all), axis=1),
                                np.expand_dims(np.array(stn_lon_all), axis=1),
                                np.array(sm_array_am_all).squeeze()), axis=1)
        sm_mat_pm = np.concatenate((np.expand_dims(np.array(stn_lat_all), axis=1),
                                np.expand_dims(np.array(stn_lon_all), axis=1),
                                np.array(sm_array_pm_all).squeeze()), axis=1)
        ...
        writer = pd.ExcelWriter(path_validation + '/' + region_name[ire] + '_' + folder_network[inw] + '_' + 'ismn_sm.xlsx')
...
for ire in range(len(folder_region)): # Region folders
    ...
    for inw in range(len(folder_network)): # Network folders
        ...
        writer = pd.ExcelWriter(path_validation + '/landcover/' + region_name[ire] + '_' + folder_network[inw] + '_' + 'landcover.xlsx')
...
####################################################################################################################################
# 2.1 Locate the SMAP 1/9 km SM positions by lat/lon of in-situ data
...
for iyr in [4]: #range(len(yearname)):
    ...
    for imo in range(len(monthname)):
        ...
        smap_file_path = path_model + '/smap_sm_9km_' + str(yearname[iyr]) + monthname[imo] + '.hdf5'
...
reg2_site = np.array([11, 13, 18, 22, 30, 31])
...
for ist in range(6):
    ...
    lns3 = ax.plot(y2, c='b', marker='o', label='9 km', markersize=5)
...
fig.text(0.04, 0.4, 'SMAP SM ($\mathregular{m^3/m^3)}$', rotation='vertical', fontsize=16, fontweight='bold')
...
fig.text(0.52, 0.01, 'Days', ha='center', fontsize=16, fontweight='bold')
fig.text(0.04, 0.4, 'SMAP SM ($\mathregular{m^3/m^3)}$', rotation='vertical', fontsize=16, fontweight='bold')
fig.text(0.95, 0.4, 'GPM Precipitation (mm/day)', rotation='vertical', fontsize=16, fontweight='bold')
plt.suptitle('Mississippi River Basin', fontsize=19, y=0.97, fontweight='bold')
