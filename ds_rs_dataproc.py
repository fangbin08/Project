...
gpm_files = gpm_files[31:] + gpm_files[0:31] # Move December 2019 to the end
...
for iyr in range(len(gpm_files_split)):
    ...
    # Save GPM data
    # os.chdir(path_model)
    var_name = ['gpm_precip_10km_' + str(yearname[iyr]), 'lat_world_geo_10km', 'lon_world_geo_10km']
    data_name = ['gpm_precip_world_10km', 'lat_world_geo_10km', 'lon_world_geo_10km']

    with h5py.File(path_model + '/gpm_precip_' + str(yearname[iyr]) + '.hdf5', 'w') as f:
        for idv in range(len(var_name)):
            f.create_dataset(var_name[idv], data=eval(data_name[idv]))
    f.close()
