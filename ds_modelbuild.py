...
lon_gldas_max = 180
lon_gldas_min = -180
# NLDAS extent corner coordinates
lat_nldas_max = 53
lat_nldas_min = 25
lon_nldas_max = -67
lon_nldas_min = -125
...
ind_timezone_max = 11
lst_am_time = 1 # 1:00
lst_pm_time = 13 # 13:00
sm_am_time = 6 # 6:00
sm_pm_time = 18 # 18:00
cellsize_12_5km = 0.125
cellsize_25km = 0.25

# Generate GLDAS/NLDAS extent lat/lon tables and corresponding row indices in the world lat table
[lat_gldas_geo_25km, lon_gldas_geo_25km] = geo_coord_gen\
    (lat_gldas_max, lat_gldas_min, lon_gldas_max, lon_gldas_min, cellsize_25km)
[lat_nldas_geo_12_5km, lon_nldas_geo_12_5km] = geo_coord_gen\
    (lat_nldas_max, lat_nldas_min, lon_nldas_max, lon_nldas_min, cellsize_12_5km)
...
# Find the corresponding UTC time zone for each latitude in the GLDAS/NLDAS lat tables
# GLDAS
...
[lst_am_utct_day, lst_am_utct, lst_am_utct_ind] = timezone_converter(lst_am_time, ind_timezone)
[lst_pm_utct_day, lst_pm_utct, lst_pm_utct_ind] = timezone_converter(lst_pm_time, ind_timezone)
[sm_am_utct_day, sm_am_utct, sm_am_utct_ind] = timezone_converter(sm_am_time, ind_timezone)
[sm_pm_utct_day, sm_pm_utct, sm_pm_utct_ind] = timezone_converter(sm_pm_time, ind_timezone)
...
# NLDAS
row_nldas_geo_12_5km_tz_ind = []
for i in range(lon_nldas_geo_12_5km.size):
    ind_min = np.absolute(lon_nldas_geo_12_5km[i] - lon_timezone_center)
    row_nldas_geo_12_5km_tz_ind.append(ind_timezone_p1[np.argmin(ind_min)])

row_nldas_geo_12_5km_tz_ind = np.asarray(row_nldas_geo_12_5km_tz_ind)
ind_timezone_nldas = np.unique(row_nldas_geo_12_5km_tz_ind)

# Group the latitudes by time zone indices
row_nldas_geo_12_5km_tz_ind_group = \
    [np.where(row_nldas_geo_12_5km_tz_ind == ind_timezone_nldas[x]) for x in range(len(ind_timezone_nldas))]

# Find out the correct corresponding UTC time (UTCT) for each time zone for specific local times (NLDAS)
[lst_am_utct_nldas_day, lst_am_utct_nldas, lst_am_utct_nldas_ind] = timezone_converter(lst_am_time, ind_timezone_nldas)
[lst_pm_utct_nldas_day, lst_pm_utct_nldas, lst_pm_utct_nldas_ind] = timezone_converter(lst_pm_time, ind_timezone_nldas)
[sm_am_utct_nldas_day, sm_am_utct_nldas, sm_am_utct_nldas_ind] = timezone_converter(sm_am_time, ind_timezone_nldas)
[sm_pm_utct_nldas_day, sm_pm_utct_nldas, sm_pm_utct_nldas_ind] = timezone_converter(sm_pm_time, ind_timezone_nldas)
...
for imo in range(len(monthnum)):
    ...
    for iyr in range(len(gldas_model_files)):
        ...
        print(str(yearname[iyr]) + '-' + monthname[imo])
    ...
    var_name = ['lst_gldas_am_delta_all_' + monthname[imo], 'lst_gldas_pm_delta_all_' + monthname[imo],
                'sm_gldas_am_all_' + monthname[imo], 'sm_gldas_pm_all_' + monthname[imo], 'ltdr_ndvi_all_' + monthname[imo]]
    data_name = ['lst_gldas_am_delta_all', 'lst_gldas_pm_delta_all', 'sm_gldas_am_all', 'sm_gldas_pm_all', 'ltdr_ndvi_all']

    with h5py.File('ds_model_' + monthname[imo] + '.hdf5', 'w') as f:
        for idv in range(len(var_name)):
            f.create_dataset(var_name[idv], data=eval(data_name[idv]))
    f.close()

    del(lst_gldas_am_delta_all, lst_gldas_pm_delta_all, sm_gldas_am_all, sm_gldas_pm_all, ltdr_ndvi_all)
    print(monthname[imo] + ' is completed')
...
for imo in range(len(monthnum)):#range(len(monthnum)):
    ...
    var_name = ['coef_mat_am_' + monthname[imo], 'metric_mat_am_' + monthname[imo],
                'coef_mat_pm_' + monthname[imo], 'metric_mat_pm_' + monthname[imo]]
    data_name = ['coef_mat_am_output', 'metric_mat_am_output', 'coef_mat_pm_output', 'metric_mat_pm_output']

    with h5py.File('ds_model_coef_' + monthname[imo] + '.hdf5', 'w') as f:
        for idv in range(len(var_name)):
            f.create_dataset(var_name[idv], data=eval(data_name[idv]))
    f.close()
...
# 4.5 Fill the NDVI classes which have no regression coefficients with coefficients from the closest class.
os.chdir(path_procdata)
...
for imo in range(len(monthnum)):
    ...
    for x in range(len(varname_list_coef)):
        ...
        if x <= 1: # Do the coefficients and intercepts filling using values from the closest class
            for m in range(var_obj.shape[0]):
                ...
                if len(ind_nan) != 0 and len(ind_nan) != var_obj.shape[1]//2:
                    ...
                    for i in range(len(ind_fill)):
                        var_obj[m, ind_nan[i]*2] = var_obj[m, ind_fill[i]*2]
                        var_obj[m, ind_nan[i]*2+1] = var_obj[m, ind_fill[i]*2+1]
        ...
        else: # Skip the R^2 and RMSE filling for each month
            pass
...
# 4.6 Make 2-d matrices and combine the regression coefficient files together
matsize_gldas_ease_1day = [len(lat_world_ease_25km), len(lon_world_ease_25km)]
mat_init = np.empty([matsize_gldas_ease_1day[0], matsize_gldas_ease_1day[1], 20], dtype='float32')
mat_init[:] = np.nan
[row_lmask_ease_25km_ind, col_lmask_ease_25km_ind] = np.where(~np.isnan(lmask_ease_25km))

for x in range(len(varname_list_coef_all)):
    mat_output = np.copy(mat_init)
    exec('var_obj =' + varname_list_coef_all[x])
    for ind in range(20):
        mat_output[row_lmask_ease_25km_ind, col_lmask_ease_25km_ind, ind] = var_obj[:, ind]
        exec(varname_list_coef_all[x] + '= mat_output')
    del(var_obj, mat_output)
